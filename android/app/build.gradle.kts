...
tasks.register("check16KBCompatibility") {
    doLast {
        println("Checking 16KB page size compatibility...")
        
        // Check compile SDK and Target SDK from flutter values
        println("NDK Version: ${flutter.ndkVersion}")
        println("Compile SDK: ${flutter.compileSdkVersion}")
        println("Target SDK: ${flutter.targetSdkVersion}")
        
        println("✓ Configuration looks compatible with 16KB page size")
    }
}

tasks.whenTaskAdded {
    if (name == "preBuild") {
        dependsOn("check16KBCompatibility")
    }
}
